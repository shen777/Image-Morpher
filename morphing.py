...
def find_Di_and_weight(point,d_line,s_line,a,b,p):
    ...
    co_X=co_P+u*(co_Q-co_P)+v*perpendicular(co_Q-co_P)/LA.norm(co_Q-co_P)
    D=co_X-X
...
def inter_line(left_lines,right_lines):
    ...
    for i in range(len(left_lines)):
        x=[(left_lines[i][0][0]+right_lines[i][0][0])/2,(left_lines[i][0][1]+right_lines[i][0][1])/2]
        y=[(left_lines[i][1][0]+right_lines[i][1][0])/2,(left_lines[i][1][1]+right_lines[i][1][1])/2]
        line=[x,y]
        lines.append(line)
    return lines
...
def bilinear(source_img,x,y,k):
    ...
    a, b = i2 -x , j2 - y
    val=(1-a)*(1-b)*source_img[i1,j1,k]+a*(1-b)*source_img[i2,j1,k]+a*b*source_img[i2,j2,k]+(1-a)*b*source_img[i1,j2,k]
    val=math.floor(val)
    return val
...
def inter_img(source_img,inter_lines,s_lines,a,b,p):
    ...
    for i in range(img1.shape[0]):
        for j in range(img1.shape[1]):
            ...
            for k in range(3):
                img[i][j][k]=bilinear(source_img,co_point[0],co_point[1],k)
    return img
...
def parameter_exp(left_lines,right_lines,img1,img2):
    ...
    for p in P:
        for a in A:
            for b in B:
                name_str="a="+str(a)+" b="+str(b)+" p="+str(p)+".png"
                print(name_str)
